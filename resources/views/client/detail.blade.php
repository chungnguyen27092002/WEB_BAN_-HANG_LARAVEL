			...
			<div class="bao_card">
				<div class="row ">
					...
					<div class="col-lg-7 single-right-left simpleCart_shelfItem">
						...
						<div class="bo">
							<?php
							$giatien = $value->product_price - $value->gia_km
							?>
							<span class="item_price"><span style="font-weight: bolder;" class="pri"></span>{{number_format($giatien)}} VNĐ</span>
							<span> @if($value->gia_km < $value->product_price && $value->gia_km > 0)
									<del>{{number_format($value->product_price)}} VNĐ</del>
									@endif
							</span>
						</div>
...
">
						...
						<div class="occasion-cart">
							<div class="snipcart-details top_brand_home_details item_add single-item hvr-outline-out">
								<form>
									@csrf
									<fieldset style="opacity: 1; position:static;justify-content:flex-start; padding-left: 0;" class="chon">
										<ul>

											<li class="mb-3">
												<p><span style="font-family:unset;font-weight: 700;">Chọn Size</span></p>
												<div class="bao2">
													@if(count($size) > 0) <!-- Kiểm tra nếu mảng $size có phần tử -->
													@foreach($size as $id => $data)
													<div class="bao3">
														<input type="radio" class="cart_product_size" name="size" value="{{$data->attribute->value}}">
														{{$data->attribute->value}}<br>
														<?php
														$tien = $value->product_price - $value->gia_km;
														$n = $tien - (($tien * 20) / 100);
														$l = $tien + (($tien * 20) / 100);
														if ($data->attribute->value == "S") {
														?>
															(<span style="color:red;font-size:13px;font-weight: bold;">{{number_format($n, 0, '.', '.')}} <span style="font-size:9px"></span></span>)
														<?php
														} elseif ($data->attribute->value == "L") {
														?>
													...
													@endforeach
													@else
													<p>Không có size</p> <!-- Thông báo nếu không có size -->
													@endif
												</div>
											...
											<li>
												<input type="button" value="@lang('lang.addToCard')" class="btn btn-nut btn-add-cart add-to-cart" data-id_product="{{$value->product_id}}" name="add-to-cart">
								...
								<form style="position: absolute; right: 300px; bottom: 125px;" action="{{ route('favorites.add', $value->product_id) }}" method="POST">
									@csrf
									<button type="submit" class="btn btn-favorite">Yêu thích</button>
								</form>
							</div>
						</div>
    ...
    <h3 class="tittle-w3l text-center mb-lg-5 mb-sm-4 mb-3 patop" style="font-size: 18px;">
        @lang('lang.sp_lienquan')
    </h3>
    <div class="cach">
        ...
        ?>
        <a href="{{ route('cli_detail', $t->product_id) }}" class="related-product">
            <div class="baoca1">
                <div class="scale-img">
                    <img width="150px" class="product-image" src="{!! asset('images/'.$t->product_image) !!}" alt="">
                </div>
                <h4 class="text-center">
                    <a class="product-title" href="{{ route('cli_detail', $t->product_id) }}">
                        {{ $t->product_name }}
                    </a>
                </h4>
                <p class="p-tien">
