...
class OrderController extends Controller
{
	...
	public function view_order($order_code, Request $request)
	{
		...
		$output['kh'] .= '<div class="khah">
				  ...
				  <ul style="list-style-type: none; line-height: 30px ; padding:0;">
		...
		<div class="vc">
		  ...
		  <ul style="list-style-type: none; padding:0;">
		    ...
		    <li><span class="tieu">Tỉnh/TP: </span>' . $shipping->shipping_address2 . '</li>
		...
		if ($shipping->shipping_method == 0) {
			$output['ship'] .= '<span class="online">Chuyển khoản qua Paypal</span>';
		} elseif ($shipping->shipping_method == 2) {
			$output['ship'] .= '<span class="online">Chuyển khoản qua VnPay</span>';
		} else {
		...
		foreach ($getorder as $key => $or) {
			if ($or->order_status == 1) {
				...
				$output['in'] .= '<option id="' . $or->order_id . '" selected value="2">Đã được giao</option>';
			}
		}
		$output['print'] = '';
	...
	public function history(Request $request)
	{
		...
		if (!Session::get('customer_id')) {
			...
			return view('Client.history')->with(compact('order', 'bra', 'cate', 'com', 'url_canonical', 'meta_desc', 'meta_title', 'share_images', 'cate_post1', 'chinh'));
		}
	}
	...
	public function view_history_order(Request $request, $order_code)
	{
		...
		if (!Session::get('customer_id')) {
			...
			$order = Order::where('customer_id', Session::get('customer_id'))->orderby('created_at', 'DESC')->get();
			return view('Client.view_history_order')->with(compact('order', 'bra', 'cate', 'com', 'url_canonical', 'meta_desc', 'meta_title', 'share_images', 'cate_post1', 'order_details', 'customer', 'shipping', 'coupon_condition', 'coupon_number', 'getorder', 'order_status', 'chinh'));
		}
	}
