...
class ProductController extends Controller
{
    ...
    {
        $pro = Product::whereHas('category', function ($query) {
            $query->where('category_status', 1);
        })->with(['category', 'brand'])->orderBy('product_id', 'DESC')->paginate(5);
    ...
    public function fetch_data(Request $req)
    {
        $pro = Product::whereHas('category', function ($query) {
            $query->where('category_status', 1);
        })->with('category')->orderBy('product_id', 'DESC')->paginate(5);
    ...
    public function imageUpload(Request $request)
    {
        if ($request->hasFile('hinh')) {
            if ($request->file('hinh')->isValid()) {
                // $request->validate([
                //     // 'hinh' => 'required|mimes:jpg,jpeg,png,gif',
    ...
    public function addpro()
    {
        ...
        return view('manager/product/add_product', compact('cate', 'attr1','bra'));
    }
    public function themsp(Request $req)
    {
        ...
        $product->brand_id = $req->brand_name;
    ...
    public function edit($id)
    {
        ...
        $bra = Brand::where('brand_status', 1)->get();
        $pro_edit = product::Find($id);
        return view('manager/product/edit_product', compact('pro_edit', 'cate','bra'));
    }
        ...
        foreach ($size as $s) {
            ...
            foreach ($size as $id => $data) {
                ...
                if ($data->attribute->value == "S") {
                    $output['product_size'] .= '<br><span style="color: #e90f0f;" class="product_nho">' . $nho . '</span>';
                } elseif ($data->attribute->value == "L") {
                    $output['product_size'] .= '<br><span style="color: #e90f0f;" class="product_lon">' . $lon . '</span>';
                } else {
                    $output['product_size'] .= '<br><span  style="color: #e90f0f;" class="product_vua">' . $vua . '</span>';
                }
